...
from zforms.fields.zhidden_input import zHiddenInput
from zforms.test.testcases import zFormTestCase


class zHiddenInputCase( zFormTestCase ):


    def setUp(self):
        super(zHiddenInputCase, self).setUp()
    ...
    def tearDown(self):
        super(zHiddenInputCase, self).tearDown()
        self.form = None
    ...
    def test_pattern_dates( self ):
        logger.debug( "\n" )
        class TestForm( forms.Form ):
            date = zHiddenInput( **{ 'pattern' : '\d{1,2}/\d{1,2}/\d{4}'  } )
