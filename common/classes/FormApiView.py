...
class FormApiView( JSONResponseMixin, BaseFormView ):
    ...
    def form_invalid( self, form ):
        ...
        return self.render_json_object_response( { "response_type": "error", "error": errors }, status=400 )
    ...
    def form_valid( self, form ):
        ...
        return self.render_json_object_response( { "response_type": "success", "data": self.object.to_dict() }, **{} )
    ...
    def get( self, request, *args, **kwargs ):
        ...
        if instance == None:
            if self.error:
                return self.render_json_object_response( { "response_type": "error",  'error': self.error }, status=404 )
            else:
                return self.render_json_object_response( { "response_type": "error",  'error': "not found" }, status=404 )
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            LogTraceback()
            return self.render_json_object_response( { 'response_type': 'error', 'error': 'invalid json' }, status=500 )
        if form.is_valid():
            logger.debug( "VALID" )
