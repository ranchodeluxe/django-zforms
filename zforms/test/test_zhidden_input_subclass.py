...
class zHiddenInputSubclassCase( zFormTestCase ):


    def setUp(self):
        super(zHiddenInputSubclassCase, self).setUp()
    ...
    def tearDown(self):
        super(zHiddenInputSubclassCase, self).tearDown()
        self.form = None
    ...
    def test_eval_hidden_input( self ):
        logger.debug( "\n========================\n" )
        self.form.fields[ 'name' ] = sHiddenInput()
    ...
    def test_default_django_field_args( self ):
        # NOTE: hidden inputs will not render labels
        logger.debug( "\n" )
        self.form.fields[ 'name' ] = sHiddenInput( **{
                'required' : True ,
                'initial' : 'stupid does' ,
    ...
    def test_pattern_dates( self ):
        logger.debug( "\n" )
        class TestForm( forms.Form ):
            date = sHiddenInput( **{ 'pattern' : '\d{1,2}/\d{1,2}/\d{4}', 'is_date' : True  } )
